...
- 단점
  1. 데이터의 추가가 어렵다
     ...
     - JavaScript, python 등 배열의 최종 길이를 지정하지 않아도 자유롭게 데이터 추가가 가능하다.
  2. 데이터의 삭제 / 수정이 복잡하다
...
- 큐의 종류
  ...
  - PriorityQueue : 우선순위 큐. 우선순위가 높은(값은 낮다) 데이터를 먼저 꺼내는 정책
...
- 단점
  ...
  1. 데이터뿐 아니라 다음 노드의 주소도 저장해야 함으로 공간효율이 나쁘다.
  ...
  - 구현 로직
    ...
    - 데이터 삭제
      ...
      1.  삭제해야 할 노드가 최하위 로드일 때
          ...
          - 노드 삭제 후 부모 노드와의 브런치를 삭제한다.

      1.  삭제해야 할 노드의 자식 노드가 1개 일때

          - 노드 삭제 후 부모 노드와 자식 노드를 연결한다.

      1.  삭제해야 할 노드의 자식 노드가 2개 일때

          - 삭제 해야 할 노드의 값에 비하여 큰값중 가장 작은 값으로 변경

            1.  삭제 해야 할 노드의 오른쪽에 있는 자식 노드들 중 가장 작은 값을 검색(=자식 노드 중 가장 왼쪽의 노드)
            1.  삭제해야 할 노드의 부모 노드의 오른쪽 자식 노드에 검색한 노드로 변경
            1.  검색한 노드의 오른쪽 자식 노드가 있을 경우, 검색한 노드의 부모 노드의 왼쪽 자식 노드와 연결정보 변경

          - 삭제 해야 할 노드의 값에 비하여 작은값중 가장 큰 값으로 변경
            1. 삭제 해야 할 노드의 왼쪽에 있는 자식 노드들 중 가장 큰 값을 검색(=자식 노드 중 가장 오른쪽의 노드)
...
- 최대힙(**MaxHeap**) : 최대값을 구하기 위한 힙구조
  - 부모 노드는 각 자식노드들 보다 크거나 같다.
- 최소합(**MinHeap**) : 최소값을 구하기 위한 힙구조
...
- 힙은 배열로 구현이 가능하다.
  ...
  - 부모 노드 인덱스 = 자식 노드 인덱스 / 2 (소수점 버림)
  - 왼쪽 자식 노드 인덱스 = 부모 노드 인덱스 \* 2
  - 오른쪽 자식 노드 인덱스 = 부모 노드 인덱스 \* 2 + 1
- 데이터 추가
  1. 배열의 마지막에 데이터를 추가
  1. 부모의 데이터와 비교하여 더 크면 변경
  1. 부모가 없거나(=루트 노드) 부모의 데이터 값이 클경우 변경하지 않는다.
- 데이터 삭제
  - 트리의 루트 데이터(최소값 또는 최대값)을 꺼내는 행위
    1. 루트 노드의 값을 꺼내고, 제일 마지막에 추가된 데이터를 루트 노드의 자리에 삽입한다.
    1. 루트 노드의 값과 자식 노드들의 값을 비교하여, 더 큰 자식 노드 값의 위치와 변경한다.
    1. 자식 노드가 없거나, 자식 노드들의 값보다 부모 노드의 값이 더 클 경우 변경하지 않는다.
